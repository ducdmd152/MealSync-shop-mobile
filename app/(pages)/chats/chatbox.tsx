...
const Chatbox = () => {
  return (
    <SafeAreaView style={{ flex: 1 }}>
      ...
      >
        <View className="flex-row items-center bg-[#e4e4e7] py-3 px-2">
          <TouchableOpacity onPress={() => router.back()} className="px-2">
            <Ionicons name="chevron-back-outline" size={28} color="gray" />
          </TouchableOpacity>
          <View className="flex-1 flex-row gap-x-2 items-center justify-center mr-8">
            <Avatar.Image
              size={22}
              source={{
            ...
            />
            <Text className="font-medium text-[20px] text-secondary text-[#f59e0b]">
              Duy Đức
            </Text>
          </View>
        </View>
        <PubNubProvider client={pubnub}>
          <Chat {...{ currentChannel, theme, users }}>
            <MessageList
              // fetchMessages={25}
              // extraActionsRenderer={(envelope) => (
              //   <Button title="Info" onPress={() => alert(`Pressed ${envelope.message.text}`)} />
              // )}
              // messageRenderer={({ message: envelope }) => <Text>{envelope.message.text}</Text>}
              // bubbleRenderer={({ message: envelope }) => <Text>Hello from {envelope.message.text}</Text>}
              // filter={(envelope) => envelope.message.text === "Render only this message differently"}
              // onScroll={(event) => alert(`Scrolled event: ${event}`)}
              style={{
            ...
            />
            <MessageInput placeholder="Gửi tin nhắn..." />
          </Chat>
        </PubNubProvider>
      </KeyboardAvoidingView>
    </SafeAreaView>
