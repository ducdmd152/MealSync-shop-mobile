...
const PromotionUpdate = () => {
  ...
  const validate = (promotion: PromotionModel) => {
    ...
    if (promotion.usageLimit < promotion.numberOfUsed)
      tempErrors.usageLimit = `Đã có ${promotion.numberOfUsed} lượt dùng, giới hạn lượt sử dụng lớn hơn hoặc bằng ${promotion.numberOfUsed}.`;
  ...
  const handleSubmit = () => {
    ...
    if (validate(promotion)) {
      apiClient
        .put("shop-owner/promotion/info/update", submitPromotion)
        .then((res) => {
          ...
          if (result.isSuccess) {
            toast.show(`Cập nhật khuyến mãi thành công.`, {
              type: "success",
              duration: 1500,
            });
